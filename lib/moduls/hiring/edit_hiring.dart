...
class EditHiring extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    nameControl.text=HiringCubit.get(context).hiringModelEdit.arabic_name??'';
    ...
    dateInterviewControl.text=HiringCubit.get(context).hiringModelEdit.date_interview??'';
   ...
   // HiringCubit.get(context).dropValue=HiringCubit.get(context).hiringModelEdit.locatwork??'SEEG';
    ...
    return BlocConsumer<HiringCubit,HiringStates>(
      ...
      if(state is HiringUpdateLoadingState){
        showDialog(
     ...
     // if(HiringCubit.get(context).messagesuccess.contains('successupdatehiring')){Navigator.of(context).pop(true);}
    },
                ...
                child: Column(children: [
                              ...
                              defaultEditText(
                                  ...
                                  validat: ( s){
                                   ...
                                   // if(s!.isEmpty) return"Mobile No is empty";
                                     ...
                                     if(!regex.hasMatch(s)&&s.toString().isNotEmpty)return'input value must be number';
                                    else if(s.length<11&&s.toString().isNotEmpty)return'Must Length  Not less than 11';
                                    else return null;
                                  }
                              ),
                            ],
                  ...
                  Row(
                    children: [
                      ...
                      // },
                         , onPress: (){
                            showDatePicker(context: context, initialDate: DateTime.now(), firstDate:
                            DateTime.parse('2010-01-01'), lastDate: DateTime.parse('2033-12-31'))
                                .then((value){
                      ...
                      Expanded(child: defaultEditText(label: 'expired ID',control: expiredIDControl,
                          ...
                          onPress: (){
                            showDatePicker(context: context, initialDate: DateTime.now(), firstDate:
                            DateTime.parse('2022-12-31') , lastDate: DateTime.parse('2033-12-31'))
                                .then((value){
                  ...
                  defaultButton(onPress: (){
                    ...
                    cubit.hiringModelEdit. expired_id=expiredIDControl.text.trim();
                          cubit.updateAllSql(context).then((value) {
                            if(cubit.messagesuccess.contains('successupdatehiring')){
                              Navigator.of(context).pop(true);
                              showDialog(
                                  barrierDismissible: false,
                                  context: context,
                                  builder:(context )=>AlertDialog(
                                    ...
                                    title: Text('Update Success',style: TextStyle(color: Colors.white),),
                                    content:  Column(
                                      mainAxisSize: MainAxisSize.min,
                                      children: [
                                        //Center(child: CircularProgressIndicator()),
                                        Text('Edit Success',style: TextStyle(color: Colors.white))
